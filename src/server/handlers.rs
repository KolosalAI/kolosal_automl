...
<body>
    ...
    function eHistScore(r){var m=r.metrics||{};var ks=['accuracy','f1','r2','auc','roc_auc','score','best_score'];for(var i=0;i<ks.length;i++){if(typeof m[ks[i]]==='number'&&m[ks[i]]>=0&&m[ks[i]]<=1)return m[ks[i]]}var vs=Object.values(m).filter(function(v){return typeof v==='number'&&v>0&&v<=1});return vs.length?vs[0]:null}
