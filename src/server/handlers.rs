...
<body>
    ...
    function eStartUmap(rawId,taskType){var modelId=rawId.replace(/[^a-zA-Z0-9_-]/g,'');var wrap=document.getElementById('umap-wrap-'+modelId);var bar=document.getElementById('umap-bar-'+modelId);if(!wrap)return;if(!window._umapSrc)window._umapSrc={};if(window._umapSrc[modelId])window._umapSrc[modelId].close();var es=new EventSource('/api/visualization/umap/stream?model_id='+encodeURIComponent(rawId));window._umapSrc[modelId]=es;es.onmessage=function(e){var d;try{d=JSON.parse(e.data)}catch(ex){return}if(d.error){if(wrap){wrap.textContent='UMAP: '+d.error}es.close();return}if(bar)bar.style.width=Math.round((d.progress||0)*100)+'%';eUmapScatter(modelId,d.task_type,d.points,d.value_range,d.done);if(d.done){var bwrap=document.getElementById('umap-bwrap-'+modelId);if(bwrap)bwrap.style.display='none';es.close()}};es.onerror=function(){var bwrap=document.getElementById('umap-bwrap-'+modelId);if(bwrap)bwrap.style.display='none';var c=document.getElementById('umap-scatter-'+modelId);if(c){var ctx=c.getContext('2d');ctx.clearRect(0,0,c.width,c.height);ctx.fillStyle='#9c9fa1';ctx.font='12px system-ui';ctx.textAlign='center';ctx.textBaseline='middle';ctx.fillText('UMAP unavailable',c.width/2,c.height/2)}es.close()}}
    var eAMLStart=0;
