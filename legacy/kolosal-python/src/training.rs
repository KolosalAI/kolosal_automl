...
impl PyTrainEngine {
    /// Get reference to inner engine (for use by other modules)
    pub fn inner_ref(&self) -> &TrainEngine {
        &self.inner
    }

    /// Create from inner engine (for use by other modules)
    pub fn from_inner(inner: TrainEngine) -> Self {
        Self { inner }
    }
}
