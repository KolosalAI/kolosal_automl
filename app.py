...
def data_upload_section():
    """Data upload and exploration section"""
...
def inference_page():
    ...
    # Input method
    input_method = st.radio("Input method", ["Upload new data", "Manual input"])
    ...
    if input_method == "Upload new data":
        ...
        if uploaded_file is not None:
            try:
                ...
                if st.button("Make Predictions"):
                    ...
                    # Make predictions
                    predictions = engine.predict(selected_model, pred_data)
        ...
        if predict_button:
            try:
                ...
                # Assuming the engine has a preprocess method
                if hasattr(engine, "preprocess"):
                    input_df = engine.preprocess(input_df)
                ...
                if engine.config.task_type == TaskType.CLASSIFICATION:
                    ...
                    # If you need probability display, you might need to modify this part
                    # to use engine.predict_proba() or similar
                else:
