...
This script compares the performance of Genta AutoML training engine
against standard manual ML approaches using pure scikit-learn without optimizations.

Features:
- Side-by-side comparison of Genta AutoML vs Standard scikit-learn
...
try:
    ...
    GENTA_AVAILABLE = True
except ImportError as e:
    print(f"Warning: Genta AutoML modules not available: {e}")
    GENTA_AVAILABLE = False
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(f"comparison_benchmark_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"),
        logging.StreamHandler()
    ]
)
logger = logging.getLogger("ComparisonBenchmark")
...
class BenchmarkResult:
    ...
    approach: str  # 'genta' or 'standard'
    dataset_name: str
    model_name: str
...
class GentaMLBenchmark:
    ...
    def run_benchmark(self, dataset_name: str, model_name: str, 
                     ...
                     device_config: str = "cpu_only", 
                     optimizer_config: str = "random_search_small") -> BenchmarkResult:
        """Run Genta AutoML benchmark."""
        if not GENTA_AVAILABLE:
            return BenchmarkResult(
                experiment_id=experiment_id,
                trial_number=trial_number,
                approach="genta",
                ...
                preprocessing_time=0,
                success=False,
                error_message="Genta AutoML modules not available"
            )
        ...
        try:
            ...
            if training_result is not None and "model" in training_result:
                ...
                # The engine should have fitted preprocessors
                ...
                if engine.feature_selector is not None:
                    X_train_processed = engine.feature_selector.transform(X_train_processed)
                    X_test_processed = engine.feature_selector.transform(X_test_processed)
                ...
                best_model_info = engine.best_model
                if best_model_info and "model" in best_model_info:
                    # Apply preprocessing for fallback case too
                    if engine.preprocessor is not None:
                        X_train_processed = engine.preprocessor.transform(X_train)
                        X_test_processed = engine.preprocessor.transform(X_test)
                    else:
                    ...
                    train_pred = best_model_info["model"].predict(X_train_processed)
                    test_pred = best_model_info["model"].predict(X_test_processed)
                else:
            ...
            # Get the model for CV (either from training result or engine's best model)
            cv_model = None
            if training_result is not None and "model" in training_result:
                cv_model = training_result["model"]
            elif engine.best_model and "model" in engine.best_model:
                cv_model = engine.best_model["model"]
            
            if cv_model is not None:
                # Use preprocessed training data for CV to match the model's expected input
                if engine.preprocessor is not None:
                    X_train_cv = engine.preprocessor.transform(X_train)
                else:
                    X_train_cv = X_train
                
                if engine.feature_selector is not None:
                    X_train_cv = engine.feature_selector.transform(X_train_cv)
            ...
            return BenchmarkResult(
                ...
                feature_count=X.shape[1],
                model_size_mb=model_size_mb,
                preprocessing_time=0,  # Included in training time for Genta
                success=True
            )
            
        except Exception as e:
            logger.error(f"Genta ML benchmark failed: {e}")
            return BenchmarkResult(
                experiment_id=experiment_id,
                trial_number=trial_number,
                approach="genta",
...
class ComparisonBenchmarkRunner:
    """Main benchmark runner comparing Genta AutoML vs Standard ML."""
    ...
    def __init__(self, output_dir: str = "./comparison_results"):
        ...
        self.genta_benchmark = GentaMLBenchmark()
    ...
    def run_single_comparison(self, dataset_name: str, model_name: str, 
        ...
        for trial in range(1, num_trials + 1):
            ...
            # Run Genta AutoML benchmark
            logger.info("Running Genta AutoML approach...")
            gc.collect()  # Clean memory before each run
            genta_result = self.genta_benchmark.run_benchmark(
                dataset_name, model_name, optimizer_config,
                experiment_id, trial, device_config, optimizer_config
            )
            
            self.results.extend([standard_result, genta_result])
            trial_results.append((standard_result, genta_result))
            
            # Log trial summary
            if standard_result.success and genta_result.success:
                speed_ratio = standard_result.training_time / genta_result.training_time if genta_result.training_time > 0 else 0
                memory_ratio = standard_result.memory_peak_mb / genta_result.memory_peak_mb if genta_result.memory_peak_mb > 0 else 0
                
                logger.info(f"Trial {trial} results:")
                logger.info(f"  Training time - Standard: {standard_result.training_time:.2f}s, Genta: {genta_result.training_time:.2f}s (ratio: {speed_ratio:.2f})")
                logger.info(f"  Test accuracy - Standard: {standard_result.test_score:.4f}, Genta: {genta_result.test_score:.4f}")
                logger.info(f"  Memory usage - Standard: {standard_result.memory_peak_mb:.1f}MB, Genta: {genta_result.memory_peak_mb:.1f}MB (ratio: {memory_ratio:.2f})")
    ...
    def generate_comparison_report(self) -> str:
        ...
        genta_results = [r for r in self.results if r.approach == "genta" and r.success]
        
        if not standard_results or not genta_results:
        ...
        html_content = self._generate_html_report(standard_results, genta_results)
    ...
    def _create_comparison_charts(self):
        ...
        fig.suptitle('Genta AutoML vs Standard ML Comprehensive Comparison', fontsize=16, fontweight='bold')
        ...
        genta_times = df[df['approach'] == 'genta']['training_time']
        if len(standard_times) > 0 and len(genta_times) > 0:
            improvement = (standard_times.mean() / genta_times.mean() - 1) * 100
            axes[0,0].text(0.5, 0.95, f'Genta is {improvement:+.1f}% faster', 
        ...
        genta_scores = df[df['approach'] == 'genta']['test_score']
        if len(standard_scores) > 0 and len(genta_scores) > 0:
            improvement = (genta_scores.mean() / standard_scores.mean() - 1) * 100
            axes[0,1].text(0.5, 0.95, f'Genta is {improvement:+.1f}% better', 
        ...
        genta_memory = df[df['approach'] == 'genta']['memory_peak_mb']
        if len(standard_memory) > 0 and len(genta_memory) > 0:
            improvement = (standard_memory.mean() / genta_memory.mean() - 1) * 100
            axes[1,0].text(0.5, 0.95, f'Genta uses {improvement:+.1f}% less memory', 
        ...
        for dataset in df['dataset'].unique():
            ...
            genta_data = dataset_df[dataset_df['approach'] == 'genta']
            
            if len(standard_data) > 0 and len(genta_data) > 0:
                time_ratio = standard_data['training_time'].mean() / genta_data['training_time'].mean()
                score_ratio = genta_data['test_score'].mean() / standard_data['test_score'].mean()
                paired_data.append({
                    'dataset': dataset,
        ...
        if paired_data:
            ...
            axes[2,0].set_ylabel('Speed Ratio (Standard/Genta)')
            ...
            axes[2,1].set_ylabel('Score Ratio (Genta/Standard)')
    ...
    def _create_scalability_chart(self, df):
        ...
        if len(genta_df) > 0:
            axes[0,0].scatter(genta_df['dataset_samples'], genta_df['training_time'], 
                             alpha=0.7, label='Genta AutoML', color='blue')
        ...
        if len(standard_df) > 0:
            axes[0,1].scatter(standard_df['dataset_samples'], standard_df['memory_peak_mb'], 
                             alpha=0.7, label='Standard ML', color='red')
        if len(genta_df) > 0:
            axes[0,1].scatter(genta_df['dataset_samples'], genta_df['memory_peak_mb'], 
                             alpha=0.7, label='Genta AutoML', color='blue')
        ...
        if len(standard_df) > 0:
            axes[1,0].scatter(standard_df['dataset_samples'], standard_df['test_score'], 
                             alpha=0.7, label='Standard ML', color='red')
        if len(genta_df) > 0:
            axes[1,0].scatter(genta_df['dataset_samples'], genta_df['test_score'], 
                             alpha=0.7, label='Genta AutoML', color='blue')
        ...
        if len(standard_df) > 0:
            ...
            axes[1,1].scatter(standard_df['dataset_samples'], standard_efficiency, 
                             alpha=0.7, label='Standard ML', color='red')
        if len(genta_df) > 0:
            genta_efficiency = genta_df['dataset_samples'] / genta_df['training_time']
            axes[1,1].scatter(genta_df['dataset_samples'], genta_efficiency, 
                             alpha=0.7, label='Genta AutoML', color='blue')
        axes[1,1].set_xlabel('Number of Samples')
    ...
    def _generate_html_report(self, standard_results: List[BenchmarkResult], 
                            genta_results: List[BenchmarkResult]) -> str:
        ...
        training_time_genta = calc_stats(genta_results, 'training_time')
        
        test_score_standard = calc_stats(standard_results, 'test_score')
        test_score_genta = calc_stats(genta_results, 'test_score')
        
        memory_standard = calc_stats(standard_results, 'memory_peak_mb')
        memory_genta = calc_stats(genta_results, 'memory_peak_mb')
        
        # Speed improvement calculation
        speed_improvement = (training_time_standard['mean'] / training_time_genta['mean'] - 1) * 100 if training_time_genta['mean'] > 0 else 0
        memory_improvement = (memory_standard['mean'] / memory_genta['mean'] - 1) * 100 if memory_genta['mean'] > 0 else 0
        accuracy_improvement = (test_score_genta['mean'] / test_score_standard['mean'] - 1) * 100 if test_score_standard['mean'] > 0 else 0
        
        html_content = f"""
        <!DOCTYPE html>
        <html>
        <head>
            <title>Genta AutoML vs Standard ML Comparison Report</title>
            <style>
        ...
        <body>
            <div class="header">
                <h1>🚀 Genta AutoML vs Standard ML Comparison Report</h1>
            ...
            <div class="summary">
                ...
                <p>This report compares the performance of <strong>Genta AutoML training engine</strong> against <strong>standard scikit-learn approaches</strong> across multiple datasets ranging from small (hundreds of samples) to massive scale (up to 10 million samples).</p>
                
                <div class="comparison-highlight">
                    <h3>🏆 Key Performance Highlights</h3>
                    <p>Based on {len(standard_results)} standard ML runs vs {len(genta_results)} Genta AutoML runs</p>
                </div>
            ...
            <table>
                ...
                <tr>
                    ...
                    <td>{training_time_genta['mean']:.2f}s</td>
                ...
                <tr>
                    ...
                    <td>{training_time_genta['std']:.2f}s</td>
                    <td>-</td>
                    <td>-</td>
                </tr>
                <tr>
                    ...
                    <td>{training_time_genta['min']:.2f}s</td>
                    <td>-</td>
                    <td>-</td>
                </tr>
                <tr>
                    ...
                    <td>{training_time_genta['max']:.2f}s</td>
                    <td>-</td>
                    <td>-</td>
                </tr>
            </table>
            ...
            <table>
                ...
                <tr>
                    ...
                    <td>{test_score_genta['mean']:.4f}</td>
        ...
        all_results = standard_results + genta_results
        ...
        if speed_improvement > 0:
            html_content += f"<li>Genta AutoML is <strong>{speed_improvement:.1f}% faster</strong> than standard ML approaches on average.</li>"
        else:
            html_content += f"<li>Standard ML is <strong>{abs(speed_improvement):.1f}% faster</strong> than Genta AutoML on average.</li>"
            
        if accuracy_improvement > 0:
            html_content += f"<li>Genta AutoML achieves <strong>{accuracy_improvement:.1f}% better accuracy</strong> than standard ML approaches on average.</li>"
        else:
            html_content += f"<li>Standard ML achieves <strong>{abs(accuracy_improvement):.1f}% better accuracy</strong> than Genta AutoML on average.</li>"
            
        if memory_improvement > 0:
            html_content += f"<li>Genta AutoML uses <strong>{memory_improvement:.1f}% less memory</strong> than standard ML approaches on average.</li>"
        else:
            html_content += f"<li>Standard ML uses <strong>{abs(memory_improvement):.1f}% less memory</strong> than Genta AutoML on average.</li>"
        
        html_content += """
                </ul>
                <p><em>Note: This comparison was conducted with Genta AutoML optimizations disabled to ensure fair comparison against standard scikit-learn approaches.</em></p>
            </div>
        </body>
        </html>
        """
    ...
    def plot_trial_results(self, save_path: str = None) -> str:
        ...
        for exp_id in df['experiment_id'].unique():
            ...
            # Genta approach
            genta_data = exp_data[exp_data['approach'] == 'genta']
            if not genta_data.empty:
                box_data.append(genta_data['test_score'].values)
                box_labels.append(f"{exp_id[:15]}..._genta")
    ...
    def generate_trial_statistics(self) -> Dict[str, Any]:
        ...
        genta_data = df[df['approach'] == 'genta']
        
        if not std_data.empty and not genta_data.empty:
            statistics['overall_comparison'] = {
                ...
                'genta': {
                    'training_time_mean': genta_data['training_time'].mean(),
                    'test_score_mean': genta_data['test_score'].mean(),
                    'memory_mean': genta_data['memory_peak_mb'].mean()
                },
                'improvements': {
                    'speed_improvement_pct': ((std_data['training_time'].mean() / genta_data['training_time'].mean()) - 1) * 100,
                    'accuracy_improvement_pct': ((genta_data['test_score'].mean() / std_data['test_score'].mean()) - 1) * 100,
                    'memory_improvement_pct': ((std_data['memory_peak_mb'].mean() / genta_data['memory_peak_mb'].mean()) - 1) * 100
                }
            }
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Compare Genta AutoML vs Standard ML")
